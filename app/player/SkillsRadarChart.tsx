    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {data.map((skill) => (
        <div key={skill.name}>
          ...
          <div style={{ width: '100%', height: 14, background: 'rgba(15, 23, 42, 0.6)', borderRadius: 10, overflow: 'hidden', border: '1px solid rgba(148, 163, 184, 0.15)' }}>
            <div style={{ width: `${Math.min(skill.value, 100)}%`, height: '100%', background: getColor(skill.value), borderRadius: 10, transition: 'width 0.4s ease' }} />
          </div>
        </div>
      ))}
    </div>
