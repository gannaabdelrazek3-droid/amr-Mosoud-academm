...
export async function DELETE(req: NextRequest) {
  try {
    ...
    await prisma.featuredPlayer.delete({ where: { id } })
    ...
    await logAudit({
      ...
      entityId: id,
      details: `حذف اللاعب المميز: ${player.name}`,
    })
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    await prisma.featuredPlayer.update({ where: { id }, data: { isActive } })
    ...
    await logAudit({
      ...
      entity: 'FeaturedPlayer',
      entityId: id,
      details: `${isActive ? 'إظهار' : 'إخفاء'} اللاعب: ${player.name}`,
    })
