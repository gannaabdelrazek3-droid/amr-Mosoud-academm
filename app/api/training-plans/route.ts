...
export async function GET(req: NextRequest) {
  ...
  const plans = await prisma.playerTrainingPlan.findMany({ where: { playerId }, orderBy: { createdAt: 'desc' } })
...
export async function POST(req: NextRequest) {
  ...
  const { playerId, title, content, eventDate } = await req.json()
  ...
  const plan = await prisma.playerTrainingPlan.create({
    data: {
      tenantId: profile.tenantId, playerId, title, content,
...
export async function DELETE(req: NextRequest) {
  ...
  const plan = await prisma.playerTrainingPlan.findUnique({ where: { id }, include: { player: true } })
  ...
  await prisma.playerTrainingPlan.delete({ where: { id } })
