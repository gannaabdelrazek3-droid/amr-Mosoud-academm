...
function EditAlbumContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/edit-album?id=${albumId}`)
      .then((res) => res.json())
      .then((data) => {
        if (!data.album) {
          setNotFound(true)
        ...
        const a = data.album
        setTitle(a.title || '')
        setCategory(a.category || 'TOURNAMENTS')
        setCoverUrl(a.coverUrl || '')
        setDisplayOrder(a.displayOrder?.toString() || '0')
        setIsActive(a.isActive ?? true)
        setItems(
          (a.items || []).map((it: { id: string; type: 'IMAGE' | 'VIDEO'; url: string; caption: string | null }) => ({
            id: it.id,
            type: it.type,
            url: it.url,
            caption: it.caption || '',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!res.ok) {
      setMessage(data.error || 'حدثت مشكلة أثناء تعديل الألبوم')
      return
    }
  ...
  if (notFound) {
    return (
      <AdminShell fullName="">
        <div style={s.page}>
          <p style={s.error}>الألبوم غير موجود</p>
        </div>
      </AdminShell>
    )
  }
    ...
    <AdminShell fullName="">
      <div style={s.page}>
        <div style={s.headerBar}>
          <div>
            <h1 style={s.title}>تعديل الألبوم</h1>
              ...
              <div key={idx} style={rowCard}>
                ...
                {it.type === 'IMAGE' ? (
                  <>
                    ...
                    {it.url && (
                      <img src={it.url} alt="معرض" style={{ width: 80, height: 80, borderRadius: 6, objectFit: 'cover', marginTop: 6 }} />
                    )}
                  </>
                ) : (
                  <>
                    ...
                    <label style={s.label}>
                      ...
                      <input type="text" value={it.url} onChange={(e) => updateItem(idx, 'url', e.target.value)} style={s.input} />
                    </label>
                  </>
                )}
                <label style={s.label}>
